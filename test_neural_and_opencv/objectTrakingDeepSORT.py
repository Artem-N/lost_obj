    ...
    def plot_boxes(self, results, frame, height, width, confidence=0.1):
        ...
        for i in range(n):
            ...
            if row[4] >= confidence:
                ...
                print(f"Detected {label} with confidence {row[4]} at [{x1}, {y1}, {x2}, {y2}]")  # Debugging print
                detections.append(([x1, y1, int(x2 - x1), int(y2 - y1)], row[4].item(), label))
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                cv2.putText(frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
        return frame, detections

cap = cv2.VideoCapture("C:\\Users\\User\\Desktop\\istockphoto-464537954-640_adpp_is.mp4")
...
while cap.isOpened():
    ...
    # print("Results:", results)  # Debugging print
