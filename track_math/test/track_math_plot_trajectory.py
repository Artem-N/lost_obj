...
def draw_line_to_center(frame, center_x, center_y, bbox_center_x, bbox_center_y):
    """Draw a line from the center of the frame to the center of the bounding box."""
    # Ensure the coordinates are integers
    bbox_center_x = int(bbox_center_x)
    bbox_center_y = int(bbox_center_y)
    cv2.line(frame, (center_x, center_y), (bbox_center_x, bbox_center_y), (0, 0, 255), 1)
    ...
    while cap.isOpened():
        ...
        if tracked_object is None and contours:
            ...
            if closest_contour is not None:
                ...
                draw_line_to_center(curr_frame, center_x, center_y, bbox_center_x, bbox_center_y)
                trajectory_points.append(last_position)
                target_lost_frames = 0
...
def main():
    ...
    process_frame(cap, kalman, prev_gray, frame_height, frame_width, CROSSHAIR_LENGTH,
                  THRESHOLD_VALUE, MIN_CONTOUR_AREA, MORPH_KERNEL_SIZE, DILATION_ITERATIONS,
                  EROSION_ITERATIONS, NUM_PREDICTIONS, MAX_TRAJECTORY_LENGTH, TARGET_MEMORY_FRAMES)
