...
def update_tracking_with_contours(contours, tracked_object, last_position, target_lost_frames, target_memory_frames, kalman, curr_frame, trajectory_points, num_predictions, is_primary):
...
def track_objects_in_frame(cap, kalman_filters, prev_gray, screen_width, screen_height, threshold_value, min_contour_area, morph_kernel_size,
    ...
    while cap.isOpened():
        ...
        tracked_objects, last_positions, lost_frames = handle_multiple_objects_tracking(
            contours, tracked_objects, last_positions, lost_frames, target_memory_frames, kalman_filters, curr_frame, min_contour_area, trajectories, num_predictions, primary_object_index
        )
...
def handle_multiple_objects_tracking(contours, tracked_objects, last_positions, lost_frames, target_memory_frames, kalman_filters, curr_frame, min_contour_area, trajectories, num_predictions, primary_object_index):
    ...
    for i in range(len(tracked_objects)):
        if tracked_objects[i] is None and contours:
            ...
            tracked_objects[i], last_positions[i], lost_frames[i] = update_tracking_with_contours(
                contours, tracked_objects[i], last_positions[i], lost_frames[i], target_memory_frames, kalman_filters[i], curr_frame, trajectories[i], num_predictions, is_primary
            )
...
def main():
    ...
    video_path = r"C:\Users\User\Desktop\fly\GENERIC_RTSP-realmonitor_2023_09_20_15_36_40.avi"
