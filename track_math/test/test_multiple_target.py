...
def update_tracking_with_contours(contours, tracked_object, last_position, lost_frames, target_memory_frames, kalman, curr_frame, trajectory_points, is_primary, initial_area):
    ...
    if contours and last_position is not None:
        ...
        if closest_contour is not None:
            ...
            # Ensure initial_area is initialized before comparison
            if initial_area is not None:
                # Check if the current object's area is close to the initial area (within a tolerance range)
                if 0.9 * initial_area < current_area < 1.1 * initial_area:  # 90% to 110% of the original area
                ...
                # Set initial_area when tracking the object for the first time
                initial_area = current_area
                last_position, bbox = update_kalman_filter(kalman, closest_contour)
                draw_bounding_box(curr_frame, bbox)  # Draw bounding box for the tracked object
    ...
    return tracked_object, last_position, lost_frames, initial_area
...
def track_objects_in_frame(cap, kalman_filters, prev_gray, screen_width, screen_height, threshold_value, min_contour_area, morph_kernel_size,
    ...
    while cap.isOpened():
        ...
        tracked_objects, last_positions, lost_frames, bbox_frame_counter = handle_multiple_objects_tracking(
            contours, tracked_objects, last_positions, lost_frames, target_memory_frames, kalman_filters, curr_frame,
            min_contour_area, trajectories, primary_object_index, bbox_frame_counter
        )
...
def handle_multiple_objects_tracking(contours, tracked_objects, last_positions, lost_frames, target_memory_frames, kalman_filters, curr_frame, min_contour_area, trajectories, primary_object_index, bbox_frame_counter):
    """Handle the logic of tracking multiple objects, removing bounding boxes 10 frames after they appear."""

    # Update bbox_frame_counter if it doesn't match the number of tracked objects
    if len(bbox_frame_counter) < len(tracked_objects):
        bbox_frame_counter += [0] * (len(tracked_objects) - len(bbox_frame_counter))
    ...
    # Track the initial area of the primary object
    initial_area = None

    for i in range(len(tracked_objects)):
        if tracked_objects[i] is None and contours:
            # Initialize tracking for the primary object or any newly detected object
            ...
            if tracked_objects[i] is not None:
                ...
                lost_frames[i] = 0
                bbox_frame_counter[i] = 0  # Reset frame counter for newly detected object
                draw_bounding_box(curr_frame, bbox, color=(255, 0, 0))  # Draw bounding box for newly detected objects

                # Set the initial area for the primary object
                if i == primary_object_index:
                    x, y, w, h = cv2.boundingRect(tracked_objects[i])
                    initial_area = w * h
        elif tracked_objects[i] is not None:
            is_primary = (i == primary_object_index)  # Check if this is the primary object
            tracked_objects[i], last_positions[i], lost_frames[i], initial_area = update_tracking_with_contours(
                contours, tracked_objects[i], last_positions[i], lost_frames[i], target_memory_frames, kalman_filters[i],
                curr_frame, trajectories[i], is_primary, initial_area
            )

            # Draw bounding box for other objects only if they are still detected and the counter is below 10
            if not is_primary and lost_frames[i] <= target_memory_frames:
                bbox_frame_counter[i] += 1
                if bbox_frame_counter[i] <= 5:  # Only draw bounding box for the first 5 frames
                    (x, y, w, h) = cv2.boundingRect(tracked_objects[i])
                    draw_bounding_box(curr_frame, (x, y, w, h),
                                      color=(0, 255, 0))  # Green bounding box for non-primary objects

            # Reset the bounding box if the object is lost for too many frames
            if lost_frames[i] > target_memory_frames:
                tracked_objects[i] = None  # Stop tracking the lost object
                last_positions[i] = None  # Reset position
                lost_frames[i] = 0  # Reset lost frames count

        # Remove bounding boxes for lost objects immediately after they disappear
        if tracked_objects[i] is None:
            last_positions[i] = None  # Reset position, and bounding box is not drawn
            bbox_frame_counter[i] = 0  # Reset the frame counter

    return tracked_objects, last_positions, lost_frames, bbox_frame_counter
...
def main():
    ...
    video_path = r"C:\Users\User\Desktop\fly\WhatsApp Video 2024-09-09 at 12.08.51.mp4"
