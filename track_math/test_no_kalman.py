...
def track_object_in_frame(cap, prev_gray, screen_width, screen_height, threshold_value, min_contour_area, morph_kernel_size,
    ...
    while cap.isOpened():
        ...
        tracked_object, last_position, target_lost_frames, last_bbox_area, last_speed = handle_object_tracking(
            contours, tracked_object, last_position, target_lost_frames, target_memory_frames, curr_frame, min_contour_area, trajectory_points, frame_center, last_bbox_area, last_speed
        )
...
def handle_object_tracking(contours, tracked_object, last_position, target_lost_frames, target_memory_frames,
    ...
    if tracked_object is None and contours: #
        tracked_object, last_position, last_bbox_area, target_lost_frames = initialize_tracking(
            contours, min_contour_area, curr_frame, last_bbox_area, target_lost_frames, trajectory_points
        )
    ...
    # Update tracking if the object is already being tracked
        ...
        tracked_object, last_position, target_lost_frames, last_bbox_area, last_speed = update_tracking_with_contours_and_speed(
            contours, tracked_object, last_position, target_lost_frames, target_memory_frames, curr_frame,
...
def initialize_tracking(contours, min_contour_area, curr_frame, last_bbox_area, target_lost_frames, trajectory_points):
    """Initialize object tracking by finding the largest contour."""
    tracked_object = find_largest_contour(contours, min_contour_area)
    if tracked_object is not None:
        # get the object's position and bounding box
        (x, y, w, h) = cv2.boundingRect(tracked_object)  # This will now be a valid call
        bbox = (x, y, w, h)
...
def update_tracking_with_contours_and_speed(contours, tracked_object, last_position, target_lost_frames,
    ...
    """handle contour matching, and check object speed and distance."""
    ...
    if contours:
        ...
        if closest_contour is not None:
            ...
            (x, y, w, h) = cv2.boundingRect(closest_contour)  # This will now be a valid call
            bbox = (x, y, w, h)
            last_position = (x + w // 2, y + h // 2)
...
def main():
    ...
    track_object_in_frame(cap, prev_gray, screen_width, screen_height, THRESHOLD_VALUE, MIN_CONTOUR_AREA, MORPH_KERNEL_SIZE,
                          DILATION_ITERATIONS, EROSION_ITERATIONS, TARGET_MEMORY_FRAMES)
